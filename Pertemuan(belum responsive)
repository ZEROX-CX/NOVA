...
<body class="bg-gray-100 min-h-screen">
    <div class="flex pt-4 container mx-auto max-w-7xl">
        <!-- SIDEBAR -->
        <aside class="w-64 bg-white shadow-md rounded-lg overflow-hidden mr-6">
            <nav class="p-2 space-y-1">
                <a href="#" class="block px-3 py-2 rounded-lg bg-indigo-100 text-indigo-700 font-medium">
                    Mapel - <?= htmlspecialchars($mapel['nama_mapel']); ?>
                </a>
            </nav>
        </aside>
        ...
        <main class="flex-1 flex">
            <div class="flex-1 bg-white shadow-md rounded-lg p-6 mr-6">
                <p class="text-sm text-gray-500 mb-2">
                   <a href="<?= BASEURL ?>/mapel_guru" class="underline">Mapel</a>  > <?= htmlspecialchars($mapel['nama_mapel']) ?>
                </p>
        ...
        <?php if (!empty($pertemuan)): ?>
            <?php foreach ($pertemuan as $p): ?>
                <div class="p-4 border rounded-lg mb-3 bg-gray-50 hover:bg-gray-100 transition">
                    <a href="<?= BASEURL ?>/pertemuan_content/<?= htmlspecialchars($mapel['id_mapel']) ?>/<?= htmlspecialchars($p['id_pertemuan']) ?>" class="font-semibold text-indigo-700"><?= htmlspecialchars($p['judul_pertemuan']) ?></a>
