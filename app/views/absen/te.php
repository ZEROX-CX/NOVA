  ...
  <body class="bg-white min-h-screen font-sans text-gray-800">
    ...
    <main class="max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row gap-6">
      ...
      <section class="flex-1">
        ...
        <h2 class="text-2xl font-bold text-purple-700 mb-6">Daftar Hadir - Pertemuan 1</h2>
            
        <!-- Form Daftar Hadir -->
        <form class="max-w-3xl mx-auto bg-white border border-purple-300 rounded-lg shadow-md p-6 space-y-6">
          <!-- Tabel Kehadiran -->
          <table class="w-full table-auto border-collapse">
            <thead>
              <tr class="bg-purple-100 text-purple-700">
                <th class="border px-4 py-2 text-left">Tanggal</th>
                <th class="border px-4 py-2 text-center">Daftar Kehadiran</th>
              </tr>
            </thead>
            <tbody>
              <!-- Contoh Baris -->
              <tr>
                <td class="border px-4 py-2">Senin, 1 Jan 2025</td>
                <td class="border px-4 py-2 text-center"><a href="<?= BASEURL ?>/absen/list">Lihat daftar</a></td>
              </tr>
              <!-- Tambahkan baris sesuai jumlah siswa -->
            </tbody>
          </table>
            ...
            <button type="button" id="tambahSesiBtn" 
                class="w-full px-4 py-2 bg-white text-purple-700 border rounded hover:bg-purple-50 focus:outline-none focus:ring-2 focus:ring-purple-500">
                + Tambah Sesi
            </button>
            </div> -->
            <script>
            const tableBody = document.querySelector("tbody");
            const tambahSesiBtn = document.getElementById("tambahSesiBtn");
            let sesiCount = 1; // bisa diubah sesuai jumlah sesi awal

            tambahSesiBtn.addEventListener("click", () => {
                sesiCount++;
                const newRow = document.createElement("tr");
                newRow.innerHTML = `
                <td class="border px-4 py-2">Pertemuan ${sesiCount} - <input type="date" name="tanggal[]" class="border rounded px-2 py-1"></td>
                <td class="border px-4 py-2 text-center"><a href="<?= BASEURL ?>/list/sesi=${sesiCount}" class="text-blue-600 hover:underline">Lihat daftar</a></td>
                `;
          ...
          <!-- Tombol Submit -->
          <!-- <button name="simpan" type="submit" class="w-full px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition">
            Simpan
          </button> -->
        </form>
