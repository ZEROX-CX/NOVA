...
<section class="flex-1 max-w-3xl">
      ...
      <div class="bg-[#7a00ff] flex w-[130px] h-[35px] items-center justify-center rounded mb-4">
        <a class="text-lg text-white inline-block" href="<?= BASEURL ?>/guru/pretest/<?= $data['id_mapel'] ?>/<?= $data['id_pertemuan'] ?>">< Kembali</a>
      </div>
        ...
        <div class="bg-[#7a00ff] text-white rounded-md p-4 mb-6 flex items-center justify-between">
            <div class="text-lg font-medium">Judul Pertemuan: <span class=""><?= htmlspecialchars($data['pertemuan']['judul_pertemuan'] ?? 'Judul Pertemuan') ?></span></div>
            <!-- Tombol Edit (Hanya untuk Guru) -->
            <?php if (isset($_SESSION['id_guru'])): ?>
                <a href="<?= BASEURL ?>/materi/edit/<?= $data['materi']['id'] ?>" class="bg-[#7a00ff] hover:bg-[#6700D8] text-white font-bold py-2 px-4 rounded text-sm transition duration-300">
                    Edit Materi
                </a>
            <?php endif; ?>
        </div>
    ...
    <div id="nextModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4 transform transition-all">
        ...
        <div class="flex gap-4 justify-end">
          ...
          <a href="<?= BASEURL ?>/guru/posttest/<?= $data['id_mapel'] ?>/<?= htmlspecialchars($pertemuan['id_pertemuan'])?>" class="px-4 py-2 bg-[#7a00ff] text-white rounded-lg hover:bg-[#6700D8] transition inline-block font-medium">
            Ya, Lanjut
          </a>
        </div>
      </div>
