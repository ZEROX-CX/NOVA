...
<body class="bg-white min-h-screen font-sans text-gray-800">
  <main class="max-w-5xl mx-auto px-4 py-10">
    <div class="bg-[#7a00ff] flex w-[130px] h-[35px] items-center justify-center rounded mb-4">
      <a href="<?= BASEURL ?>/guru/materi/<?= $data['id_mapel'] ?>/<?= $id_pertemuan ?>" class="text-lg text-white inline-block">&lt; kembali</a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 border border-gray-200">
      ...
      <div class="bg-[#7a00ff] text-white rounded-md p-4 mb-6 flex items-center justify-between">
        ...
        <?php if ($isGuru): ?>
          <a href="<?= BASEURL ?>/posttest/create/<?= $data['id_mapel'] ?>/<?= $id_pertemuan ?>" class="bg-[#7a00ff] hover:bg-[#6700D8] text-white font-bold py-2 px-4 rounded text-sm transition duration-300">
            + Tambah Soal Baru
          </a>
        <?php endif; ?>
      </div>
        ...
        <form method="post" action="<?= BASEURL ?>/posttest/submit" class="space-y-6">
          ...
          <?php $no = 1; foreach ($questions as $row): ?>
            <div class="bg-gray-50 border border-gray-300 rounded-md p-4 relative">
              ...
              <?php if ($isGuru): ?>
                <div class="absolute top-2 right-2 space-x-2">
                  <a href="<?= BASEURL ?>/posttest/edit/<?= $row['id_posttest'] ?>" class="bg-[#7a00ff] hover:bg-[#6700D8] text-white text-xs font-bold py-1 px-2 rounded transition duration-300">
                    Edit
                  </a>
                  <a href="<?= BASEURL ?>/posttest/delete/<?= $row['id_posttest'] ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus soal ini?');" class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-1 px-2 rounded transition duration-300">
                    Hapus
                  </a>
                </div>
              <?php endif; ?>
            ...
            <div class="flex justify-end">
              <a href="<?= BASEURL ?>/guru/nilai_posttest/<?= $data['id_mapel'] ?>/<?= $data['id_pertemuan'] ?>" class="px-4 py-2 bg-[#7a00ff] text-white rounded hover:bg-[#6700D8] mr-2 transition">Lihat Nilai siswa</a>
            </div>
        </form>
      <?php else: ?>
