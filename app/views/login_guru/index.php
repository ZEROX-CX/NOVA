<?php
    ...
    if(isset($_SESSION["is_login"])) {
      header("Location: " . BASEURL . "/beranda/new");
      exit;
    }
    ...
    if(isset($_POST['kirim']))  {
        ...
        $sql = "SELECT * FROM guru WHERE nama_guru='$namaGuru' AND password='$hash_password'";
        $result = $db->query($sql);
...
<head>
  ...
  <style>
    ...
    /* HILANGKAN WARNA KUNING SAAT TAP */
    input, button, a {
...
bg-gradient-to-br from-[#7a00ff] via-[#8f2bff] to-[#5b00c7]">
  ...
  <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden z-10">
    ...
    <div class="relative flex bg-gray-100 mt-4">
      <a href="#" id="tabSiswa"
         class="w-1/2 text-center py-3 font-semibold text-gray-500">Siswa</a>
      <a href="#" id="tabGuru"
         class="w-1/2 text-center py-3 font-semibold text-purple-700">Guru</a>
    ...
    <div class="p-6 sm:p-8">
      ...
      <form action="" method="POST" class="space-y-5">
        <input type="text" name="namaGuru" placeholder="Username" required
...
<script>
  ...
  /* TAB PINDAH */
  document.getElementById("tabSiswa").onclick = e => {
    e.preventDefault();
    document.getElementById("activeLine").style.left = "0%";
    setTimeout(() => location.href = "login_siswa", 300);
  };
